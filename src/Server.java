...
public class Server {
    ...
    private void createsANewOnlineGame(ClientHandler p1, ClientHandler p2){
        ...
        sendToUserApproveGame(p1, isP1White);
        sendToUserApproveGame(p2, !isP1White);
    }

    // Send to a user that the game is started.
    private void sendToUserApproveGame(ClientHandler targetClient, boolean isWhite){

        Data message = new Data(ActivityType.APPROVE_TO_PLAY, "server", targetClient.getClientUserName(), convertBooleanIsWhite(isWhite));
        try {
